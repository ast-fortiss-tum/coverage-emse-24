...
from statistical_tests import wilcoxon
...
def write_analysis_results(result_runs_all, save_folder):
    ...
    ratio_critical_both_average = np.sum(
        ratio_critical_single_run[algo2]) / np.sum(ratio_critical_single_run[algo1])
    ratio_critical_both_average_no_duplicates = np.sum(
        ratio_critical_single_run_no_duplicates[algo2]) / np.sum(ratio_critical_single_run_no_duplicates[algo1])
    ...
    with open(save_folder + f'analysis_combined.csv', 'w', encoding='UTF8', newline='') as f:
        ...
        write_to.writerow(
            [f'Critical Scenarios {algo1}', np.sum(n_critical_all[algo1])])
        write_to.writerow([f'Critical Scenarios {algo1} (duplicate free)', np.sum(
            n_critical_all_no_duplicates[algo1])])

        write_to.writerow(
            [f'Critical Scenarios {algo2}', np.sum(n_critical_all[algo2])])
        write_to.writerow([f'Critical Scenarios {algo2} (duplicate free)', np.sum(
            n_critical_all_no_duplicates[algo2])])

        write_to.writerow(
            [f'Ratio Critical Scenarios {algo2}/{algo1} (union)', '{0:.2f}'.format(ratio_critical_both)])
        write_to.writerow(
            [f'Ratio Critical Scenarios {algo2}/{algo1} (union, duplicate free)', '{0:.2f}'.format(ratio_critical_both_no_duplicates)])

        write_to.writerow(
            [f'Ratio Critical Scenarios {algo2}/{algo1} (average)', '{0:.2f}'.format(ratio_critical_both_average)])
        write_to.writerow([f'Ratio Critical Scenarios {algo2}/{algo1} (average, duplicate free)',
...
def make_comparison_plot(max_evaluations, 
    ...
    if shift_error is True:
        offset_algo_x = 0.025 * max_evaluations
        offset_algo_y = 0.015
        marker_error = ''
    else: 
        offset_algo_x = 0
...
def retrieve_metric_data_from_csv(paths, n_algos, ids=None):
    print(f"[retrieve_metric_data_from_csv] paths: {paths}")
    storing_arrays = []
...
def retrieve_metric_data_from_csv_damir(paths, n_algos):
    storing_arrays = []
    ...
    for key, path in enumerate(paths):
        ...
        for i in range(n_algos):
            metric_algo = [
                        [x*pop_size for x in table.iloc[:, 0].values.tolist()], 
                        table.iloc[:,2*i + 1].values.tolist(), 
                        table.iloc[:, 2*i + 2].values.tolist()
            ]
            by_metric.append(metric_algo)
        storing_arrays.append(by_metric)
...
def statistical_analysis(metric_name_load, 
    ...
    for algo in algos:
        ...
        for algo in m_bases:
            log.info(m_bases[algo])

        log.info(m_test)
        p_val, effect =  wilcoxon.run_wilcoxon_and_delaney(m_test,m_bases)
    ...
    with open(save_folder + f'{metric_name_label}_significance.csv', 'w', encoding='UTF8', newline='') as f:
        ...
        for algo in algos:
            if algo == algo_test:
                continue
            write_to.writerow([f'{algo}', results[algo][0],   results[algo][0]])
        f.close()
...
def statistical_analysis_from_overview(metric_name, 
    ...
    file_name = input_folder + f"overview_{metric_name}{suffix if suffix is not None else ''}.csv"
