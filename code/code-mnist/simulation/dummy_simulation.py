...
class DummySimulator(Simulator):
    time_step = 1
    DETECTION_THRESH = 1    # threshold in meters where other actors can be detected
    RANDOMNESS_BIAS = 0.2   # noise to be added to positions
    ...
    @staticmethod
    def simulate_orientation_fixed(list_individuals, 
                     variable_names, 
                 scenario_path: str, 
                 sim_time: float, 
                 time_step: float = time_step,
        ...
        for ind in list_individuals:
            new_ind = [0, *ind]
            simout = DummySimulator.simulate_single(new_ind, 
                                                    variable_names, 
                                                    filepath=scenario_path, 
                                                    sim_time=sim_time,
                                                    time_step=time_step)
